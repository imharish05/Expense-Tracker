...
// Custom Table CSS for Mobile View
...
const TreasuryLayer = () => {
  ...
  const [form, setForm] = useState({ amount: "", source: "Union Bank", beneficiary: "", description: "" });
  ...
  const handleTopUp = async () => {
    ...
    toast.promise(action, {
      ...
      success: () => {
        setForm({ amount: "", source: "Union Bank", beneficiary: "", description: "" });
        fetchTreasuryStatus(); // Refresh data after success
      ...
      <div className="container">
        ...
        <div className="row g-4 mt-2">
          <div className="col-12 col-lg-8 order-2 order-lg-1">
                      ...
                      return (
                        <div key={src} className="col-12 col-md-4">
                          <div className="h-100 p-3" style={{
                            ...
                            <div className="d-flex align-items-center gap-2 mb-md-1">
            ...
            {/* Audit Trail */}
            <div style={{ ...heroCardStyle, padding: 0 }} className="mt-3">
                ...
                <table className="table table-hover align-middle mb-0 responsive-audit-table">
                  ...
                  <tbody>
                    ...
                    {!loading && data.recentLogs.length === 0 && (
                      <tr>
                        <td colSpan="5" className="text-center py-4 text-muted small">No transactions found.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
          ...
          <div className="col-12 col-lg-4 order-1 order-lg-2">
            <div style={heroCardStyle}>
              ...
              <div style={{ position: 'relative', zIndex: 1 }}>
                    ...
                    {VALID_SOURCES.map((src) => {
                      ...
                      return (
                        <button key={src} type="button" className="btn btn-sm text-start fw-bold border-2"
                          onClick={() => setForm({ ...form, source: src, beneficiary: "" })}
                          style={{
                ...
                <div style={{
                  overflow: "hidden", maxHeight: form.source ? "110px" : "0px", opacity: form.source ? 1 : 0,
                  transition: "max-height 0.32s ease, opacity 0.25s ease", marginBottom: form.source ? "0.75rem" : 0,
                }}>
                  ...
                  <div className="position-relative">
                    <input type="text" className="form-control border-0" placeholder="Enter name"
                      style={{ borderRadius: 12, fontSize: 14, paddingLeft: "2.4rem", backgroundColor: selectedColor?.light || "#f8fafc", color: selectedColor?.text || "#1e293b", padding: '12px 12px 12px 2.4rem' }}
                      value={form.beneficiary} onChange={(e) => setForm({ ...form, beneficiary: e.target.value })}
                    />
                    <Icon icon="solar:user-bold-duotone" width="16" style={{ position: "absolute", left: "0.75rem", top: "50%", transform: "translateY(-50%)", color: selectedColor?.primary || "#94a3b8" }} />
                  </div>
                </div>

                <div className="mb-4">
                  <label className="small fw-bold text-secondary mb-1 d-block" style={{ fontSize: 10 }}>NOTE</label>
                  <input type="text" className="form-control border-0 bg-light" placeholder="Description"
                    style={{ borderRadius: 12, fontSize: 14, padding: '12px' }} value={form.description}
                    onChange={(e) => setForm({ ...form, description: e.target.value })}
                  />
                </div>

                <button className="btn w-100 py-3 my-3 fw-bold text-white shadow-sm" onClick={handleTopUp}
                    ...
                    return (
                      <div key={src} className="mb-3">
                        ...
                        <div className="progress" style={{ height: 6, background: "#f1f5f9", borderRadius: 10 }}>
                          <div className="progress-bar rounded-pill" style={{ width: `${pct}%`, backgroundColor: c.primary }} />
                        </div>
                      </div>
                    );
                  })}
